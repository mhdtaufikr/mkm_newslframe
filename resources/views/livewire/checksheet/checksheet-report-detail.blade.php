                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    @if($inspection->serial_number)
                        <div>
                            ...
                            <p class="text-sm font-bold text-slate-800 mt-1 font-mono">{{ $inspection->serial_number }}</p>
                        </div>
                    @endif
                    <div>
            ...
            <div class="space-y-4">
                @foreach($sections as $sectionId => $results)
                    @php
                        ...
                        $meta       = $sectionMeta[$sectionId] ?? ['type' => 'standard', 'ngTypes' => []];
                        $type       = $meta['type'];
                        $ngTypes    = $meta['ngTypes'];
                        $isWelding  = $type === 'welding';
                        $isPainting = $type === 'painting';
                    @endphp
                    ...
                    <div class="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                        ...
                        <div class="px-4 py-3 bg-slate-100">
                            <div class="flex items-center justify-between">
                                ...
                                @if($isWelding)
                                    <span class="text-xs font-bold px-2 py-1 rounded-lg bg-blue-100 text-blue-700">🔧 Welding</span>
                                @elseif($isPainting)
                                    <span class="text-xs font-bold px-2 py-1 rounded-lg bg-purple-100 text-purple-700">🎨 Painting</span>
                                @endif
                            </div>
                        </div>
                                ...
                                <thead>
                                    @if($isWelding)
                                        <tr class="bg-slate-50 border-b border-slate-200">
                                            ...
                                            <th class="px-3 py-2 text-left text-xs font-bold text-slate-700 uppercase" rowspan="2">REMARKS</th>
                                        </tr>
                                        <tr class="bg-slate-50 border-b border-slate-200">
                                    ...
                                    @elseif($isPainting)
                                        <tr class="bg-slate-50 border-b border-slate-200">
                                            ...
                                            <th class="px-3 py-2 text-center text-xs font-bold text-slate-700 uppercase w-20">RESULT</th>
                                            @foreach($ngTypes as $ngType)
                                                <th class="px-3 py-2 text-center text-xs font-bold text-red-600 uppercase w-20">
                                                    {{ strtoupper($ngType) }}
                                                </th>
                                            @endforeach
                                            <th class="px-3 py-2 text-center text-xs font-bold text-blue-600 uppercase w-20">REPAIR</th>
                                            <th class="px-3 py-2 text-left text-xs font-bold text-slate-700 uppercase">REMARKS</th>
                                        </tr>
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @foreach($results as $index => $result)
                                        @php
                                            ...
                                            $repairValue = $resultData['repair'] ?? null;
                                        @endphp
                                        ...
                                        <tr class="hover:bg-slate-50 transition-colors {{ $result->result === 'ng' ? 'bg-red-50/50' : '' }}">
                                            ...
                                            {{-- NG Breakdown — welding & painting --}}
                                            @if($isWelding || $isPainting)
                                                @foreach($ngTypes as $ngType)
                                            ...
                                            {{-- Repair — painting saja --}}
                                            @if($isPainting)
                                                <td class="px-3 py-2 text-center">
                                                    @if($repairValue === 'yes')
                                                        <span class="inline-flex items-center justify-center px-3 py-1 bg-blue-100 text-blue-700 rounded-lg text-xs font-bold">
                                                            YES
                                                        </span>
                                                    @elseif($repairValue === 'no')
                                                        <span class="inline-flex items-center justify-center px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-xs font-bold">
                                                            NO
                                                        </span>
                                                    @else
                                                        <span class="text-slate-300">-</span>
